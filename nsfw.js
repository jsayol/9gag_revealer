...
function gag_reveal_nsfw() {
	...
	for (var i=0; i<els.length; i++) {
		...
		if (el.getAttribute('alt') == 'NSFW') {
			...
			// let's add an event to detect when the image can't be loaded, so some ectra processing can be done
			if (!el.getAttribute('_9gagrevealer')) {
				el.setAttribute('_9gagrevealer', true);
				el.onerror = (function() {
					// first revert back to the original NSFW placeholder to hide the "broken image" icon
					this.el.setAttribute('src', this.url);

					// then make an AJAX call to the post's URL and pass its response to the callback
					var ajaxo = new ajaxObject('/gag/'+this.id, (function(rTxt,rStat) { gag_reveal_youtube(this.parent, this.id, rTxt, rStat); }).bind(this));
					ajaxo.update();
